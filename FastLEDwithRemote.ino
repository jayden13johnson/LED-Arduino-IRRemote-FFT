...
void fillFFT() {
  for (int i = 0; i < SAMPLES; i++) {
    ...
    while ((micros() - newTime) < sampling_period_us) {
      /* chill */
    } //Waiting until enough time has elapsed (~100uS) so that another sample is able to be taken
  }
  ...
  //Actually assigning our new RGB values to the lights
  int i;
  ...
  //Resetting values in bandValues[] array
  int j;
